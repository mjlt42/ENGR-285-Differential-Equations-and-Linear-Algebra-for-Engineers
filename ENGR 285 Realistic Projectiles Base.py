...
def f(t, u):
    ...
    y = u[2]
    vy = u[3]
    return [vx, -k*((vx**2 + vy**2)**0.5)*vx, vy, -g - k*((vx**2 + vy**2)**0.5)*vy]
...
for p in range(iterations):
    ...
    step = 0.001
    k = 1 #Drag constant (for the system)
    ...
    vx_initial = 4 #Starting x velocity (for the system)
    vy_initial = 5 #Starting y velocity (for the system)

    launchAngleUse = True #Set to True if launched at an angle or set to False to set each velocity separately
    angle = pi/4 #Launch angle in radians (0 < angle < π/2)
    v_initial = 1 #Launch Speed

    if launchAngleUse:
        vx_initial = v_initial*cos(angle) #Starting x velocity at an angle (for the system)
        vy_initial = v_initial*sin(angle) #Starting y velocity at an angle (for the system)
    ...
    errorRange = abs(Range2 - Range1)/2 #Decreasing step size reduces the error of the range
    ...
    with open(file_path, 'w') as file:
        ...
        file.write(f"Drag constant: {k} 1/m\n")
